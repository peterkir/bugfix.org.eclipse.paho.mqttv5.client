...
└── .github/
    ├── scripts/
    │   ├── ci-build.sh          # build + publish to dist
    │   └── sonatype-upload.sh   # upload dist to Sonatype Central Portal
    └── workflows/
...
2. Runs `.github/scripts/ci-build.sh` which:
   ...
   - Runs `./gradlew build` (with
     signing when GPG secrets are present).
3. Runs `.github/scripts/sonatype-upload.sh dist` when `SONATYPE_BEARER`
   is available, uploading the contents of `dist` to Sonatype Central
...
./.github/scripts/sonatype-upload.sh dist
```

After upload the script prints the Deployment ID and stores it in
`dist_DEPLOYMENTID.txt`. Log in to <https://central.sonatype.com> to
